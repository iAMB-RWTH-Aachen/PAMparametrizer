...
from PAModelpy.utils.pam_generation import _order_enzyme_complex_id

@pytest.mark.parametrize("sheet_name, enzyme_id, expected_kcat_f",
                         [#1 enzyme per reaction, constant reaction annotation
                             ('case_01',
                              'E1_E2_E3',
                              1004),
                             #1 enzyme per reaction, reaction annotation can be CE or rxn_id
                             ('case_02',
                              'E1_E2_E3',
                              1004),
...
def test_if_create_pamodel_from_diagnostics_file_is_correct(sheet_name:str,
                                                            enzyme_id:str,
                                                            expected_kcat_f:float):
    # Arrange
...
def build_toy_model(sensitivity:bool=True, membrane_sector: bool=False):
    ...
    model = load_json_model(os.path.join('tests', 'models', 'toy_model.json'))
