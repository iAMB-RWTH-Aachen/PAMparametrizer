        ...
        while (g < self.number_generations) and ((time()-start_time) < self.time_limit):
            ...
            for mutant in offspring:
                ...
                if random.random() < self.mutation_probability:
                    # mutate an individual with a mutation rate
            ...
            fitnesses = map(toolbox.evaluate, invalid_ind,) #[model for i in range(len(invalid_ind))])
            for ind, fit in zip(invalid_ind, fitnesses):
                ind.fitness.values = fit
                # store fitness
                fitness_dict[ind_str(ind)] = fit
