...
#         rxn_id = _extract_reaction_id(rxn.id) #some reactions ids are associated with copy numbers, only filter for the actual reaction id
#         if not (
...
def create_pamodel_from_diagnostics_file(file_path:str, model: PAModel)-> PAModel:
    ...
    for _, group in best_individual_df.groupby('run_id'):
        for _, row in group.iterrows():
            ...
            model.change_kcat_value(enzyme_id=row['enzyme_id'], kcats=kcat_dict)
    return model