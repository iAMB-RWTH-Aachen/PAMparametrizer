...
def plot_fva_bargraph(fva_result: pd.DataFrame,
    ...
    plt.savefig(os.path.join('Results', '3_analysis','fva_central_carbon_metabolism_iML1515.png'))
...
if __name__ == '__main__':
    ...
    fva_result = pd.concat(
        [df.reset_index(names = 'rxn').assign(model=name) for name, df in pd.read_excel(FVA_RESULT_FILE, sheet_name=None, index_col=0).items()],
        ignore_index=True
    )
    fva_result = fva_result[fva_result['rxn'].isin(rxns_to_plot)]
    print(fva_result.to_markdown())
    plot_fva_bargraph(fva_result=fva_result, rxns_to_plot=rxns_to_plot)
