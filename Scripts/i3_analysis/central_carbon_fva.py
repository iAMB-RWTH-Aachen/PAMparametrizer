...
from Modules.utils.pam_generation import setup_pputida_pam, setup_cglutanicum_pam, create_pamodel_from_diagnostics_file
...
def main_datareduc():
    NUM_MODELS = 1
    GLC_UPTAKE_FLUXOMICS = 9.67
    FVA_RESULT_FILE = os.path.join('Results', '3_analysis', 'fva_central_carbon_metabolism_datared.xlsx')
    FIG_RESULT_FILE = os.path.join('Results', '3_analysis', 'fva_central_carbon_metabolism_datared.png')
    rxns_to_plot = ['GLCptspp', 'HEX1', 'PGI', 'PFK', 'F6PA', 'TALA', 'FBA', 'GAPD', 'PDH', 'CS', 'OAADC', 'ACONTa',
                    'ACONTb', 'ICDHyr', 'SUCOAS', 'SUCDi', 'FUM', 'MDH', 'ME2']
    models_to_check = {f'Alternative_{i}': os.path.join('Results', '3_analysis', 'parameter_files',
                                                        f'proteinAllocationModel_EnzymaticData_iML1515_{i}.xlsx')
                       for i
                       in range(1, NUM_MODELS + 1)}
    # models_to_check = {}
    models_to_check['GotEnzymes'] = (
        os.path.join('Results', '2_parametrization', 'proteinAllocationModel_iML1515_EnzymaticData_multi.xlsx'))
    models_to_check = {
        name: set_up_pam(file, sensitivity=False)
        for name, file in models_to_check.items()
    }
    models_to_check['10perc_data'] = create_pamodel_from_diagnostics_file(
        'Results/data_reduction_results/diagnostics/pam_parametrizer_diagnostics_datareduc_10_5.xlsx',
        models_to_check['GotEnzymes'].copy(copy_with_pickle=True))
    for model in models_to_check.values():
        model.change_reaction_bounds('EX_glc__D_e', -GLC_UPTAKE_FLUXOMICS, 0)

    models_to_check['iML1515'] = read_sbml_model(os.path.join('Models', 'iML1515.xml'))
    models_to_check[
        'iML1515'].reactions.EX_glc__D_e.lower_bound = -GLC_UPTAKE_FLUXOMICS
    ...
    plot_fva_bargraph(fva_result=fva_result,
                      rxns_to_plot=rxns_to_plot,
                      fig_result_file=FIG_RESULT_FILE,
                      other_colors={'GotEnzymes': 'grey', 'iML1515': 'purple', '10perc_data':'red'})
