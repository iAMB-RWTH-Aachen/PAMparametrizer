...
def plot_exchange_fluxes_vs_experiments(all_fluxes: pd.DataFrame,
    ...
    all_fluxes = all_fluxes.sort_values(by='substrate', ascending=False)
    all_alternative_ids = [f'alternative {i}' for i in range(1,11)]
    cmap = {
            **{'iML1515': 'black', 'GotEnzymes': 'black', 'Randomized': 'midnightblue', 'Curated': 'black'},
    ...
    if axs is None:
        ...
        for r, ax in zip(reactions_to_plot, axs):
            ...
            for model in model_order:
                ...
                ax.plot(flux_df['substrate'].abs(), flux_df['flux'].abs(),
                        color = cmap[model],
        ...
        for handle, label in zip(h, l):
            ...
            labels.append(label)
        handles = ([handles[0]]+
                   [Line2D([0], [0], color='lightblue', alpha = 0.8),
                    Line2D([0], [0], color = 'indianred')]
                   +handles[1:]
                   )
        labels = [labels[0]]+['Alternative PAMs', 'Randomized PAMs']+labels[1:]
